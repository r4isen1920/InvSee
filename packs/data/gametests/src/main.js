...
function fetchInventory(entity) {
	...
	let inventory = Array.from(
		...
		(_, i) => container.getItem(i) || { typeId: 'air' }
	);
...
function projectPlayerInventory(player, sourcePlayer) {
	...
	const entityProjector = sourcePlayer.dimension.spawnEntity(
		'r4isen1920_invsee:inventory',
		sourcePlayer.location
	);
	...
	for (let i = 45; i < 53; i++) {
		if (equipment[i - 45]?.typeId !== 'air')
			entityProjectorContainer.setItem(i, equipment[i - 45]);
		else continue;
	}

	sourcePlayer.runCommand(
		`ride @s start_riding @e[type=r4isen1920_invsee:inventory,c=1] teleport_ride`
	);
	entityProjector.addTag('invsee');
	entityProjector.setDynamicProperty('r4isen1920_invsee:target', player.id);

	player.setDynamicProperty(
		'r4isen1920_invsee:old_log',
		stringifyInventory(inventory.concat(equipment))
	);
	entityProjector.setDynamicProperty(
		'r4isen1920_invsee:old_log',
		stringifyInventory(fetchInventory(entityProjector))
	);
}
...
function getInventoryViewerEntities(dimension) {
	return world
		...
		.getEntities({ type: 'r4isen1920_invsee:inventory', tags: ['invsee'] });
}
...
function checkSync(entityProjector) {
	...
	const playerInventory = stringifyInventory(
		fetchInventory(target).concat(fetchPlayerEquipments(target))
	);
	...
	if (playerInventory !== target.getDynamicProperty('r4isen1920_invsee:old_log')) {
		...
		viewerInventory !== entityProjector.getDynamicProperty('r4isen1920_invsee:old_log')
	) {
...
function pushChange(entityProjector, target) {
	...
	for (let i = 0; i < 36; i++) {
		if (viewerInventorySlice[i].typeId !== 'air')
			targetContainer.setItem(i, viewerInventorySlice[i]);
		else continue;
	}
	for (let i = 45; i < 53; i++) {
		if (viewerEquipment[i - 45].typeId !== 'air')
			targetEquipment.setEquipment(indexToEquipment[i], viewerEquipment[i - 45]);
		else continue;
	}
}
...
function showSearchUI(player) {
	...
	ui.show(player).then((result) => {
		...
		if (filteredPlayers.length === 0) {
			...
			ui.title({ translate: 'gui.invsee.search' }).body({
				translate: `gui.invsee.matchFound.${filteredPlayers.length > 1 ? 'plural' : 'singular'}`
			});
